...
from infrastructure.config import (
    ...
    FINNHUB_KEY, TARGET_COMPANIES, SEC_HEADERS, GEMINI_KEYS, BROWSER_HEADERS
)
...
HARD_COLUMN_TAGS = {
    "revenue": [
        "InterestAndNoninterestIncome", # Total Revenue for many banks
        "InterestAndDividendIncomeOperating", # WAL specific Total Interest Income
        "InterestIncomeNet",             # Net Interest Income
        "NetInterestIncome",
        "Revenues", 
        "InterestIncomeLoss",            # Total Interest Income
        "NoninterestIncome",
        "TotalRevenues",
        ...
        "RevenueFromContractWithCustomerExcludingAssessedTax", # Subset, deprioritized
    ],
    "net_income": [
        "ProfitLoss", 
        "NetIncomeLoss", 
    ...
    "operating_income": [
        "IncomeLossFromContinuingOperationsBeforeIncomeTaxes", # Common for banks
        "OperatingIncomeLoss", 
...
class SECDataMatcher:
    ...
    def extract_points(self, tag_data: dict) -> dict:
        ...
        # Support various SEC units. We iterate through all unit keys to find usable data.
        # Common keys: "USD", "pure", "shares", "USD/shares", "INR", "EUR", etc.
        for u_key, unit_entries in units.items():
            # Skip non-financial units if we already have a better one? 
            # Actually, let's just process the first one that has 10-K/10-Q forms.
            # If multiple exist, the loop will naturally pick the first one it encounters.
            ...
            if result: break # Once we find a unit that works for these dates, we stick with it
        return result
...
def ingest_sec_ticker(ticker: str, meta: dict, sb: Client, matcher: SECDataMatcher):
    ...
    try:
        ...
        for d, info in all_dates.items():
            ...
            if "/" in unit:
                unit = unit.split("/")[0]
            if unit in ["pure", "shares", "USD/shares"]:
                unit = "USD"
            ...
            for raw_facts in [data_gaap, data_ifrs]:
                for tag, tag_data in raw_facts.items():
                    if tag in tags_actually_used: continue
            ...
            # --- Robust Banking Revenue Synthesis ---
            curr_rev = row.get("revenue", 0) or 0
            
            # Create a temporary view of all metrics for this date
            all_metrics = row.get("flex_metrics", {}).copy()
            if curr_rev > 0:
                # Map the current revenue back to the tags that produced it
                for t in tags_actually_used:
                    all_metrics[t] = curr_rev
            
            # 1. Interest Income
            ii = all_metrics.get("InterestAndDividendIncomeOperating") or all_metrics.get("InterestIncomeLoss") or all_metrics.get("InterestIncome")
            
            # 2. Non-interest Income
            non_ii = all_metrics.get("NoninterestIncome")
            
            # 3. Net Interest Income (as backup)
            nii = all_metrics.get("NetInterestIncome") or all_metrics.get("InterestIncomeLossNet") or all_metrics.get("InterestIncomeNet") or all_metrics.get("InterestIncomeExpenseNet")
            
            # 4. Synthesize Total Revenue (Banks report Net Revenue = NII + Non-interest Income)
            sum_revenue = 0
            if nii and non_ii:
                sum_revenue = nii + non_ii
            elif ii and non_ii:
                sum_revenue = ii + non_ii
            
            # 5. Final assignment (Banks prefer NII + Non-interest even if smaller than gross interest)
            if sum_revenue > 0:
                if nii and non_ii:
                    row["revenue"] = sum_revenue
                    log.info(f"[{ticker}] Banking Net Revenue synthesized for {d}: {sum_revenue}")
                elif sum_revenue > curr_rev:
                    row["revenue"] = sum_revenue
                    log.info(f"[{ticker}] Revenue synthesized for {d}: {sum_revenue}")
            ...
            # --- 3.5 Sanity Check: Revenue >= Net Income ---
            if row.get("revenue") is not None and row.get("net_income") is not None:
                if row["revenue"] < row["net_income"]:
                    log.warning(f"[{ticker}] Revenue ({row['revenue']}) < Net Income ({row['net_income']}). Attempting correction...")
                    # If revenue is too small, maybe we picked a sub-item. 
                    # Try to see if we can find 'InterestAndNoninterestIncome' or similar in flex_metrics
                    alt_revenue = None
                    for alt_tag in ["InterestAndNoninterestIncome", "InterestIncomeNet", "NetInterestIncome", "InterestIncomeExpenseAfterProvisionForLoanLosses"]:
                        if alt_tag in row.get("flex_metrics", {}):
                            alt_revenue = row["flex_metrics"][alt_tag]
                            break
                    
                    if alt_revenue and (row.get("revenue") is None or alt_revenue > row["revenue"]):
                        row["revenue"] = alt_revenue
...
def clean_financials(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for row in rows:
        ...
        for k, v in row.items():
            if k in ["ticker", "company_name", "currency", "fiscal_period", "end_date", "data_source", "sec_filing_url", "filing_type", "archived_url"]:
                r[k] = v

        # Fix integer columns
        if "year" in row:
            try: r["fiscal_year"] = int(float(row["year"]))
            except: pass
        if "fiscal_year" in row:
            try: r["fiscal_year"] = int(float(row["fiscal_year"]))
            except: pass
...
def analyze_with_gemini(text: str, file_bytes: bytes = None, mime_type: str = "application/pdf") -> Dict[str, Any]:
    instruction = f"""You are a high-precision financial data extraction and normalization engine. 
Your task is to extract exact financial metrics from unstructured documents (PDFs, transcripts, or web pages).

## STEP 1: VALUE EXTRACTION & HIERARCHY
Extract values for: Total Revenue, Net Income (PAT), Operating Income (EBIT), Cash & Equivalents, EPS (Diluted), Total Assets, Total Equity, Total Liabilities.

### BANKING SECTOR SPECIFIC RULES:
- If the company is a BANK or FINANCIAL INSTITUTION:
    - REVENUE (Total Revenue) = "Total Interest and Noninterest Income". If that is missing, sum "Interest Income" and "Noninterest Income".
    - DO NOT use "Revenue from Contracts" or "Fee Income" as Total Revenue if Interest Income exists.
    - OPERATING INCOME = "Income Before Income Taxes" or "Operating Profit".
    - Traditional "Sales" will likely be zero; do NOT return zero if Interest Income exists.
- PRIORITY: Use 'Consolidated' figures over 'Standalone'.
- If a value is missing or labeled as "–" or "nil", return null.
- NEVER guess or calculate missing values yourself unless specifically instructed.

## STEP 2: UNIT & MULTIPLIER NORMALIZATION
Convert all text-based numbers into absolute floats:
- "1.5 Billion" -> 1500000000
- "200 Million" -> 200000000
- "50 Crore" -> 500000000
- "10 Lakh" -> 1000000
- "1.2T" -> 1200000000000
- Handle spoken phrases: "32 like 19 thousand" -> 3219000.

## STEP 3: DYNAMIC CURRENCY DETECTION
Identify the currency of the reported numbers. 
- LOOK for symbols: ₹ -> "INR", $ -> "USD", € -> "EUR", £ -> "GBP".
- CAUTION: "R" or "Rs." often refers to Indian Rupees (INR). DO NOT assume "R" is South African Rand (ZAR) unless the company is explicitly South African.
- CRITICAL: If the document mentions 'Crore', 'Lakh', 'Cr', or 'L', the currency is 100% Indian Rupees (INR).
- LOOK for text: "USD", "INR", "Rupees", "Dollars", "Euros".
- Output the 3-letter ISO code (e.g., "USD", "INR", "EUR"). 
- If not found, look at the company name/sector to infer the most likely currency (e.g., Indian companies -> INR).

## STEP 4: MULTI-YEAR & PERIOD DETECTION
- If the document provides a table with multiple years (e.g., 2023, 2024, 2025), you MUST return an entry for each year in the 'financials' array.
- Identify the 'end_date' for each period (YYYY-MM-DD). If only a year is given, use YYYY-03-31 for Indian companies and YYYY-12-31 for others.

## STEP 5: VALIDATION
1. Consistency check: Total Assets should be approximately Total Liabilities + Total Equity.
2. Margin check: Net Margin = (Net Income / Total Revenue) * 100. If result > 100%, re-verify numbers.
...
{{
  ...
  "insights": "Executive summary of performance",
  "tables": ["Markdown version of original data tables if found"]
}}
"""
    ...
    from infrastructure.config import get_gemini_key
    try:
...
class ExtractorEngine:
    ...
    def process(self, input_source, filename=None, ticker_override=None, company_override=None, currency_override=None, push_to_supabase=True):
        ...
        for fin in financials_input:
            ...
            f_currency = currency_override or fin.get("currency")
...
def main():
    import argparse
    from infrastructure.config import get_company_meta, get_supabase
    ...
    if args.ticker:
        ...
        from infrastructure.config import load_target_companies
